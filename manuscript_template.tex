...
\hypersetup{
  pdftitle={A Hidden Markov Framework for Time-to-Flare Prediction in Crohn's Disease Under Endogenous Laboratory Sampling: A Simulation Study},
  pdfauthor={Rishi Jasti}
}
...
\author{\uppercase{Rishi Jasti}\authorrefmark{1}}
\address[1]{North Carolina School of Science and Mathematics, Durham, NC 27705 USA (e-mail: jasti27r@ncssm.edu)}
\tfootnote{}
\markboth{Jasti: Hidden Markov Framework for Time-to-Flare Prediction in Crohn's Disease}
{Jasti: Hidden Markov Framework for Time-to-Flare Prediction in Crohn's Disease}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{fig1_graphicalmodel.pdf}
...
Each of @@N_SEEDS@@ seeds generates @@N_TOTAL@@ patients. The first @@N_TRAIN@@ patients are used for training, @@N_VAL@@ are reserved as an untouched validation partition, and @@N_TEST@@ are used for final evaluation. The validation partition was not used in the reported analysis; all settings used for the reported reconstruction are recorded in the archived configuration. Each patient contributes @@N_DAYS@@ observed days. An additional @@FUTURE_DAYS@@ latent days are simulated solely to assess long-horizon interval coverage; no future observation enters fitting or prediction. The numerical PMF is calculated through day @@PMF_HORIZON@@, with all remaining mass stored in a $>@@PMF_HORIZON@@$ tail category.
...
Versioned source code, complete predictive distributions, fitted model objects, generated tables and figures, and the locked computational environment are archived at Zenodo, DOI: @@PUBLIC_ARCHIVE_DOI@@ \cite{jasti2026code}. The corresponding source is available from GitHub release \texttt{@@RELEASE_TAG@@}.
...
\bibitem{jasti2026code} R. Jasti, ``A Hidden Markov Framework for Time-to-Flare Prediction in Crohn's Disease Under Endogenous Laboratory Sampling: Code and Reproducibility Materials,'' version @@SOFTWARE_VERSION@@, Zenodo, 2026, doi: @@PUBLIC_ARCHIVE_DOI@@.
\end{thebibliography}

\begin{IEEEbiographynophoto}{Rishi Jasti}
is a student researcher at the North Carolina School of Science and Mathematics, Durham, NC, USA. His research interests include probability theory, stochastic modeling, and simulation-based disease progression modeling.
